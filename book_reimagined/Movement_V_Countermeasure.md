...
**Source:** old Ch29-32. Per the outline's post-pilot correction: V.1-V.2 (Ch29-30) do NOT use a full scene-level interleave — the `Pilot_V1V2_Ch29_Ch30_Interleave.md` test found that bet doesn't pay off the way III.23's did, because Ch30 consumes Ch29's output rather than running parallel to it. Instead: Ch29 compressed ~15% (concentrated in the Selim/Emre calls), followed by one short new bridge marking simultaneity, then Ch30 reused whole. V.3-V.4 (Ch31-32) reused, with two "It is the ___" texture fixes applied during the IV-V verification pass: Ch32's "It is the answer I have" → "I don't have a cleaner one", and a fifth instance found only during verification, Ch31's "It is the answer supported by the contract" → "The contract doesn't give me a better one" (this one sat in the same chapter as Torres's "It is the answer I can sign," making it the most clustered pair of the five total instances). Two instances remain untouched (Ch19, Ch31's Torres line) as an intentional-reading echo rather than full elimination.

---

## V.1 — Unmodeled (Ch29, compressed ~15%, concentrated in the Selim/Emre calls)
...
## V.2 — Adaptive Variance (Ch30, reused verbatim)
...
## V.3 — Exposure Window (Ch31, reused verbatim)
...
## V.4 — Polaris (Ch32, reused, with the "Would you do it again" exchange rewritten so Cate gives a direct answer instead of another dodge)
...
- Movement V follows the pilot's own negative finding instead of ignoring it: no forced scene-level interleave between Ch29 and Ch30. The one new bridge paragraph is short, marks simultaneity honestly, and doesn't pretend to be more clever than the material supports.
- V.3-V.4's texture fix (varying the closest "It is the ___ I ___" repeat) is applied; the other three instances across the book (Ch19, Ch31, Ch32's first) stay as an intentional-reading echo rather than getting swept away entirely.
- The book's ending is untouched: Ch32 closes exactly as before, on Naomi's risk record silently reclassifying her, deliberately open.

## Open items for review

- V.1's compression (~15% target) is by feel, same caveat as every other movement's percentages.
- ~~The whole reimagined book is now drafted end to end... cross-movement seams haven't specifically been checked against each other.~~ **Done, 2026-08-09** — full front-to-back read of the reading manuscript (all 34 chapters), specifically checking seams around every insertion this session (assassin team, Zhou, Daniel Cho, physical descriptions, Elif attack intercut). Found and fixed: one missing physical description (Daniel Cho), and separately, a real reader-notes/Movement-file sync-drift bug (see [[book-reimagined-workflow]]) affecting ~15 spots plus one missing bridge paragraph in this file, now restored. The five movements do cohere as one continuous book.
